...
const common = {
  ...
  lnb: function () {
    const target = '.left-nav';
    const link = target + ' .lnb-link';
    const lnbBtn = target + ' .lnb-sidebar-btn';
    $(document).on('click',link,function(){
      $(this).toggleClass('close').siblings('ul').slideToggle(300);
      return false;
    });

    $(document).on('click',lnbBtn,function(){
      $(this).closest('#tkg-container').toggleClass('lnb-close');
      return false;
    });
  },
  footer: function () {},
